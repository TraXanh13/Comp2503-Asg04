 ...
 *
 * It uses Binary Search Trees to store the Word objects. It uses iterator
 * objects to access the data in the BST.
 * 
...
public class A4 {
	/*
	 * The lists (trees) of words. List (tree) of unique words, ordered
	 * alphabetically List (tree) of keywords, ordered by the keyword comparator
	...
	/**
	 * Print the list of keywords. Use the implemented iterator to traverse the tree
	 * in-order
	 */
	private void printKeywords() {
		for (Word w : keywordsByFreqDesc) {
			System.out.println(w.getWord() + " : " + w.getCount());
	}
	}

	/**
	 * Print the list of longwords. Use the implemented iterator to traverse the
	 * tree in order
	 */
	private void printLongwords() {
		for (Word w : longwordsByLengthDesc) {
				System.out.println(w.getWord() + " : " + w.getCount());
		}
	}
	...
	private int avgFreq() {
		...
		avg = avg / keywordsByFreqDesc.size();
		return (int) avg;
	}
	...
	private int avgLength() {
		double avg = 0;
		for(Word w : longwordsByLengthDesc) {
			avg += w.getWord().length();
		}
		avg = avg / longwordsByLengthDesc.size();
		return (int) avg;
	}

	/*
	 * Calculate the optimal height for a tree of size n. Round to an int.
	 */
	private int optHeight(int n) {
		double h = 0;
		if (n > 0) {
			h = (Math.log(n + 1) / Math.log(2));
			return (int) h;
		}
		return -1;
	}
